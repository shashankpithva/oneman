...
(function(){
  ...
  var seen=false;try{seen=sessionStorage.getItem('oneman_boot')==='1';}catch(e){}
  function finish(){
    boot.classList.add('done');
    document.body.style.overflow='';
    try{sessionStorage.setItem('oneman_boot','1');}catch(e){}
  ...
  function addLine(){
    ...
    var pfx=document.createElement('span');pfx.className='pfx';pfx.textContent='oneman:~$ ';
  ...
  function banner(){
    ...
    row.innerHTML='<span class="ok">\u2713</span> welcome to <b>OneMan</b>';
    body.appendChild(row);
...
(function(){
  ...
  const TW=[
    ...
    ['AutoSeal','autoseal','Notary services are still running on pen and paper. AutoSeal brings AI dispatch, scheduling, and document management to the notary industry. Built in a day on OneMan.'],
    ...
    ['Carta Mira','cartamira','Shipped a full multilingual storefront today — EN, ES, PT. OneMan wrote every line and deployed it live.'],
  ...
  function makeNode(who,handle,body){
    ...
    d.innerHTML='<div class="top"><div class="av">'+initial(who)+'</div><div><div class="who">'+who+'</div><div class="handle">@'+handle+'.oneman.app</div></div><div class="ago">'+rnd(AGOS)+'</div></div><p>'+body+'</p><div class="meta"><span>&#9825; '+(Math.floor(Math.random()*120)+3)+'</span><span>&#8635; '+Math.floor(Math.random()*40)+'</span><span>&#8599;</span></div>';
    return d;
  }
  ...
  function tick(){
    ...
    else{const n=rnd(NAMES);who=n;handle=n.toLowerCase().replace(/[^a-z]/g,'');body=n+' '+rnd(SUFX)+'. Built and run on OneMan.';}
