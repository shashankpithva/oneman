...
This lets Polsia publish approved posts straight to your X account. You hit **Approve** in the Outbox and the post goes live — no compose window.
...
3. Open your app → **User authentication settings** → **Set up** / **Edit**:
   ...
   - **Callback URI / Redirect URL:** paste your app page URL exactly:
     ```
     https://shashankpithva.github.io/polsia/app.html
     ```
   - **Website URL:** `https://shashankpithva.github.io/polsia/`
   - Save.
...
The Worker code is in this repo at `worker/polsia-x-worker.js`.

**Easiest (dashboard):**
1. Go to **https://dash.cloudflare.com** → sign up (free, no card).
2. **Workers & Pages** → **Create** → **Create Worker** → give it a name → **Deploy**.
3. Click **Edit code**, delete the sample, paste the entire contents of `worker/polsia-x-worker.js`, then **Deploy**.
4. Copy your Worker URL — it looks like:
   ```
   https://polsia-x.YOUR-NAME.workers.dev
   ```
...
npm i -g wrangler
wrangler login
wrangler deploy worker/polsia-x-worker.js --name polsia-x
```

---

## Step 3 — Connect it in Polsia (~30 sec)
...
That's it. Now when Polsia drafts an X post into the **Outbox**, just click **Post on X** / **Approve** and it publishes automatically.
